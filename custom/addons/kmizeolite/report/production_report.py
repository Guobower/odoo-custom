...
class Reportproduction(models.AbstractModel):
    _name = 'report.kmizeolite.print_report_production'
    ...
    def _get_product_types(self, data, use_domain):
        ...
        domain = []
        if use_domain == 1:
            domain = [('state', 'in', ['sale', 'done']), ('invoice_status', 'in' , ['to invoice', 'invoiced']), ('invoiced_date', '>', data.get('from_date')), ('invoiced_date', '<', data.get('to_date'))]
        elif use_domain == 2:
            domain = [('state', '=', 'done'), ('invoice_status', '=' , 'invoiced'), ('invoiced_date', '>', data.get('from_date')), ('invoiced_date', '<', data.get('to_date'))]
    ...
    def render_html(self, docids, data=None):
        ...
        return self.env['report'].render('kmizeolite.print_report_production', docargs)
